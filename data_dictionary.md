...
| Field | Type | Description | Example |
|-------|------|-------------|---------|
| CRSDepTime | STRING | Scheduled departure (hhmm) | 0840 |

### Delay Fields (Key Fields for Analysis)

| Field | Type | Description | NULL Handling | Example |
|-------|------|-------------|---------------|---------|
| DepDelay | FLOAT | Actual minus scheduled departure (min). Negative = early | Replace NULL with 0 | 4.0 |
| ArrDelay | FLOAT | Actual minus scheduled arrival (min). Negative = early | Replace NULL with 0 | -29.0 |
| DepDel15 | FLOAT | 1 if departure delay >= 15 min | Replace NULL with 0 | 0.0 |
| ArrDel15 | FLOAT | 1 if arrival delay >= 15 min (primary target for ML) | Replace NULL with 0 | 0.0 |

### Cancellation and Diversion Fields

| Field | Type | Description | Example |
|-------|------|-------------|---------|
| Cancelled | FLOAT | 1.0 = flight cancelled | 0.0 |
| CancellationCode | STRING | A=Carrier, B=Weather, C=NAS, D=Security, NONE=not cancelled | NONE |
| Diverted | FLOAT | 1.0 = flight diverted to different airport | 0.0 |

### Delay Cause Fields
> These are only populated when ArrDelay > 0.
...
| Field | Type | Description | Example |
|-------|------|-------------|---------|
| Distance | FLOAT | Distance between airports (miles) | 2442.0 |
| DistanceGroup | INT | Distance bucket (every 250 miles) | 10 |
| TaxiOut | FLOAT | Gate to takeoff time (minutes) | 34.0 |
| TaxiIn | FLOAT | Landing to gate time (minutes) | 4.0 |
| AirTime | FLOAT | Actual flight time in air (minutes) | 246.0 |
...
| Column shifting | Trailing comma at end of each row adds extra field | Python detects and removes trailing empty column |
| Airport names with commas | San Francisco, CA splits into two columns in CSV | Pipe separator used instead of comma |
...
| ArrDel15/DepDel15 as FLOAT | BTS exports binary flags as 0.00 and 1.00 | Hive FLOAT type accepts both; use = 1.0 in WHERE |
...
|---------|---------|---------|
| DepHour | CRSDepTime / 100 (integer) | Hour of day (0-23) |
| IsPeakHour | 1 if DepHour between 16 and 20 else 0 | Peak evening hours flag |
| IsWeekend | 1 if DayOfWeek in (6, 7) else 0 | Weekend travel flag |
| IsDelayed | 1 if ArrDel15 = 1 else 0 | ML target variable |
