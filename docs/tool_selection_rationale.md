...
**Key fixes applied:**
- Hive 1.2 does not support OTHERWISE in CASE statements — use ELSE
- NVL() function replaces all NULL values in aggregations
- OpenCSVSerde used for cancellation_codes to strip quoted values
- Beeline used instead of Ambari to avoid unicode encoding issues
...
Queries run in under 2 seconds with full SQL expressiveness.
