...
HDFS /user/maria_dev/flight_data/cleaned/
    ...
    |           - 4 tables created (flights, airports, codes, carriers)
    |           - 7 analytical queries
    |           - Results: delays, cancellations, carriers, hourly trends
    |
...
(including em dashes and smart quotes) when SQL is
copy-pasted from external sources. These cause
ParseException errors that do not appear in Beeline.
Beeline connects directly to HiveServer2 over JDBC
without any text encoding issues.
...
mislead booking decisions. HBase's CP (Consistent +
Partition tolerant) model guarantees accurate reads
at the cost of some availability during network partitions.
...
prevents silent truncation of fractional delay minutes
and avoids NULL values from failed integer casting.
...
|-------|-----------|------------|
| All text fields NULL in Pig output | BTS CSV uses quoted format, Pig read whole lines as single column | Switched to Python for cleaning |
| Column shifting in output | Trailing comma at end of each CSV row added phantom column | Python strips trailing empty field |
| Wrong column values | Incorrect column indices used in Python script | Printed all 120 column names with indices and mapped correctly |
| Hive query returns 0 rows | Cleaned folder already existed — Hive LOCATION pointed to old empty data | Re-uploaded clean file to HDFS |
| AVG delay = 1488 minutes | ArrDelay column was actually reading ArrTimeBlk (time block string) | Fixed column indices in Python script |
| Cancellation JOIN returns empty | L_CANCELLATION.csv stored as "A","Carrier" with embedded quotes | Used OpenCSVSerde to strip quotes |
| Hive ParseException | Ambari copy-paste adds hidden unicode characters | Switched all complex queries to Beeline |
...
|-- scripts/
|   |-- python/
|   |   |-- clean_flights.py              Data cleaning script
|   |-- hive/
|   |   |-- create_tables.hql             DDL for all 4 Hive tables
...
|
|-- notebooks/
|   |-- BTS_Flight_Delay_Visualizations.ipynb    5 chart visualizations
|   |-- BTS_Flight_Delay_Spark_ML.ipynb          ML model and results
|
|-- visualizations/
...
|
|-- docs/
|   |-- tool_selection_rationale.md       Why each tool was chosen
|   |-- pipeline_architecture.md          This document
|   |-- data_dictionary.md                All 34 fields explained
|
|-- data/
|   |-- samples/
|   |   |-- sample_5rows.csv              5-row sample for testing
```
